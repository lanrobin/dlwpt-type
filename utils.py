...
def load_data_jay_lyrics():
    """加载周杰伦歌词数据集"""
    with zipfile.ZipFile(f'{DOWNLOAD_DATA_ROOT}/download_data/jaychou_lyrics.txt.zip') as zin:
...
# ######################################## 7.3 ###############################################
def get_data_ch7():  
    data = np.genfromtxt(f'{DOWNLOAD_DATA_ROOT}/download_data/airfoil_self_noise.dat', delimiter='\t')
...
def load_data_pikachu(batch_size, edge_size=256, data_dir = f'{DOWNLOAD_DATA_ROOT}/download_data/pikachu'):  
    """edge_size：输出图像的宽和高"""
...
# ################################# 9.9 #########################
def read_voc_images(root=f"{DOWNLOAD_DATA_ROOT}/download_data/VOCdevkit/VOC2012", 
